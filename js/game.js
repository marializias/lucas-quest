...
function saveHitboxOverrides(){
  ...
  window.addEventListener("keydown", (e) => {
    ...
    if(["arrowup","arrowdown","arrowleft","arrowright"," ","e","enter"].includes(key)){
      e.preventDefault();
    }
  });
  ...
  function handlePuzzleClick(x,y){
    ...
    if(puzzle.type === "drawerChoice"){
      [300,480,660].forEach((bx,index)=>{
        if(x>=bx-70 && x<=bx+70 && y>=245 && y<=350){
          puzzle.cursor=index;
          press("e");
        }
      });
...
if (state.screen === "menu") {
  ...
  function updatePuzzle(){
    ...
    if(puzzle.type==="drawerChoice"){
      if(consume("arrowleft","a")) puzzle.cursor=Math.max(0,puzzle.cursor-1);
      if(consume("arrowright","d")) puzzle.cursor=Math.min(2,puzzle.cursor+1);
      if(consume("e"," ","enter")){
        puzzle.opened[puzzle.cursor]=true;
        if(puzzle.cursor===0) say("Gaveta esquerda: só toalhas de mesa.",150);
        if(puzzle.cursor===2) say("Gaveta direita: contas antigas.",150);
        if(puzzle.cursor===1){
          state.activePuzzle=null;
